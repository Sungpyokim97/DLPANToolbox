...
%           Q2n_index:          Q2n index.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [psnr, mssim, Q_index, SAM_index, ERGAS_index, sCC, Q2n_index] = indexes_evaluation(I_F,I_GT,ratio,L,Q_blocks_size,flag_cut_bounds,dim_cut,th_values)
...
Q_index = Q(I_GT,I_F,2^L-1);
SAM_index = SAM(I_GT,I_F);
ERGAS_index = ERGAS(I_GT,I_F,ratio);
sCC = SCC(I_F,I_GT);
[mssim, ~] = multi_channel_ssim(I_GT, I_F, K, window, 2^L-1);
psnr = PSNR(I_GT, I_F, 2^L-1);
cd ..
